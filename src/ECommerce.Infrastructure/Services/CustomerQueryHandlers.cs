...
public sealed class CustomerQueryHandlers : 
    IRequestHandler<SearchCustomersQuery, Result<PagedResponse<CustomerResponse>>>,
    IRequestHandler<GetCustomerDetailsQuery, Result<CustomerDetailsResponse>>
{
    private readonly ApplicationDbContext _context;

    public CustomerQueryHandlers(ApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<Result<PagedResponse<CustomerResponse>>> Handle(SearchCustomersQuery command, CancellationToken cancellationToken)
    {
        var searchTerm = command.SearchTerm;
    ...
    public async Task<Result<CustomerDetailsResponse>> Handle(GetCustomerDetailsQuery command, CancellationToken cancellationToken)
    {
