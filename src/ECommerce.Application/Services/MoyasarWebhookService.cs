...
public sealed class MoyasarWebhookService : IMoyasarWebhookService
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IOrderRepository _orderRepository;
    private readonly IProcessedWebhookEventRepository _webhookEventRepository;
    private readonly IStockService _stockService;
    private readonly ILogger<MoyasarWebhookService> _logger;

    public MoyasarWebhookService(
        IUnitOfWork unitOfWork,
        IOrderRepository orderRepository,
        IProcessedWebhookEventRepository webhookEventRepository,
        IStockService stockService,
        ILogger<MoyasarWebhookService> logger)
    {
    ...
    public async Task<Result<WebhookAckResponse>> ProcessWebhookAsync(MoyasarWebhookPayload payload, string configuredSecret, CancellationToken cancellationToken = default)
    {
        ...
        // 3. Process inside ExecuteInTransactionAsync — compatible with NpgsqlRetryingExecutionStrategy
        try
        {
            return await _unitOfWork.ExecuteInTransactionAsync<Result<WebhookAckResponse>>(async ct =>
            {
                ...
                // 4. Record Webhook Event (idempotency token — within same transaction)
